			  ...
			  onSelect: option.onSelect||function(files){
				  ...
				  for(var i=0;i<files.length;i++){
					  if(files[i].size > 838860800){
						  alert("文件大小超过限制，不能大于800MB");
						  return;
					  }
					  size1+=files[i].size;
				}
				  if((parseInt(size1) + parseInt($("#used").val())) > 5368709120){
					  alert("上传文件大小超过总容量");
					  return;
				  }
				 for(var i=0;i<files.length;i++){
						  ...
						  function get_filemd5sum(ofile,ofid,xhr1) {
							  ...
							  if(file.size<2097152000){
								  fileReader.onload = function(e) {
									  ...
									  fileReader.abort();
									  $.ajax({
										  url : "http://47.110.83.145:8086/file/md5Check",
										  type : "get",
										  ...
										  success : function(data) {
											 // alert(data)
											  if (data.respCode==1) {
												  $.ajax({
													  url : "http://47.110.83.145:8086/file/quickUploadFile",
													  type : "post",
