...
function exit() {
    $("#my-confirm").modal({
        closeViaDimmer: 0, onConfirm: function (b) {
            $.ajax({
                url: "http://47.110.83.145:8086/user/logout?token=" + $.cookie("token"),
                type: "GET",
                xhrFields: {withCredentials: true},
                crossDomain: true,
                success: function (c) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.respData.indexOf(",") > -1) {
                ...
                $(".share-url").val("http://47.110.83.145:8097/s/" + data.respData.substring(0, data.respData.lastIndexOf(",")));
                $(".share-password").val(data.respData.substring(data.respData.lastIndexOf(",") + 1, data.respData.length));
                $(".copyPrivate").val("http://47.110.83.145:8097/s/" + (data.respData.substring(0, data.respData.lastIndexOf(","))) + " 提取密码:" + data.respData.substring(data.respData.lastIndexOf(",") + 1, data.respData.length))
            } else {
                $(".share-validity-tip").css("right", "115px");
                $(".create-link").addClass("public-link has-create");
                $("#createShare").css("display", "none");
                $("#cel").children("span").children("span").html("关闭");
                $(".share-url").val("http://47.110.83.145:8097/s/" + data.respData)
            }
        },
        error: function () {
            alert("服务器错误")
