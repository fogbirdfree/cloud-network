...
function loadCapacity() {
    $.ajax({
        url: "http://47.110.83.145:8086/core/useCapacity?uid=" + $('#uid').val(),
        type: "get",
...
function ajax() {
    var a = $(this).data("key");
    if (a == "all") {
        location.href = "http://47.110.83.145:8097/disk/home" + "#/" + a + "?path=/"
    } else {
        if (a == "myShare") {
            location.href = "http://47.110.83.145:8097/share/manage"
        } else {
            location.href = "http://47.110.83.145:8097/disk/home" + "#/" + a
        }
    }
}
...
function loadContent() {
    ...
    $.ajax({
        url: "http://47.110.83.145:8086/share/shareList",
        type: "get",
        data: {"uid": $('#uid').val(), "page": 1, "order": a, "desc": b},
        xhrFields: {withCredentials: true},
        crossDomain: true,
        dataType: "json",
        success: function (c) {
...
function loadContent1(b) {
    ...
    $.ajax({
        url: "http://47.110.83.145:8086/share/shareList",
        type: "get",
        data: {"uid": $('#uid').val(), "page": b, "order": a, "desc": c},
        xhrFields: {withCredentials: true},
        crossDomain: true,
        dataType: "json",
        success: function (d) {
...
function exit() {
    $("#my-confirm").modal({
        closeViaDimmer: 0, onConfirm: function (b) {
            $.ajax({
                url: "http://47.110.83.145:8082/user/logout?token=" + $.cookie("token"),
                type: "GET",
                xhrFields: {withCredentials: true},
                crossDomain: true,
                success: function (c) {
                ...
                $.ajax({
                    ...
                    success: function (g) {
                        if (g.respCode == 0) {
                            ...
                            $.ajax({
                                url: "http://47.110.83.145:8082/user/modifyPassword",
                                type: "post",
                                data: JSON.stringify({
                                    "token": $.cookie("token"),
                                    "newPassword": c,
        ...
        closeViaDimmer: 0, onConfirm: function (c) {
            ...
            if (e != "PNG" && e != "GIF" && e != "JPG" && e != "JPEG" && e != "BMP") {
                ...
                if (parseInt(b) >= parseInt(g)) {
                    ...
                    f.append("file", d);
                    $.ajax({
                        url: "http://47.110.83.145:8082/user/uploadPic",
                        type: "post",
                        contentType: false,
                        data: f,
                        cache: false,
...
function loadImg() {
    $.ajax({
        url: "http://47.110.83.145:8082/user/loadImg?uid=" + $('#uid').val(),
        type: "GET",
        xhrFields: {withCredentials: true},
        crossDomain: true,
        success: function (b) {
...
function unShare(a) {
    $.ajax({
        url: "http://47.110.83.145:8086/share/unShare",
        type: "get",
        data: {"uid": $('#uid').val(), "vid": JSON.stringify(a)},
        xhrFields: {withCredentials: true},
        crossDomain: true,
        dataType: "json",
        success: function (b) {
