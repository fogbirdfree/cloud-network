...
function loadContent3(b, c) {
    $.ajax({
        url: "http://47.110.83.145:8086/core/searchFile?uid=" + $("#uid").val() + "&key=" + b + "&page=" + c + "&order=update_time",
        type: "get",
        xhrFields: {withCredentials: true},
        crossDomain: true,
        success: function (d) {
...
function loadContent2(b) {
    $(".vdAfKMb").empty();
    $.ajax({
        url: "http://47.110.83.145:8086/core/searchFile?uid=" + $("#uid").val() + "&key=" + b + "&page=" + 1 + "&order=update_time",
        type: "get",
        xhrFields: {withCredentials: true},
        crossDomain: true,
        success: function (d) {
...
function loadContent() {
    ...
    $.ajax({
        url: "http://47.110.83.145:8086/core/listFile",
        type: "get",
        data: {
            "uid": $("#uid").val(),
            "type": h,
...
function loadFolder(c, d) {
    ...
    if (b == null) {
        $.ajax({
            url: "http://47.110.83.145:8086/core/listFolder",
            type: "get",
            data: {"uid": $("#uid").val(), "parentPath": c},
            xhrFields: {withCredentials: true},
            crossDomain: true,
            dataType: "json",
            success: function (f) {
...
function copyAndMove(id, chooseNum, dest) {
    ...
    for (var index in chooseNum) {
        ...
        vid.push(contentVal.uuid)
    }
    $.ajax({
        url: "http://47.110.83.145:8086/core/copyOrMoveFile",
        type: "put",
        data: JSON.stringify({"uid": $("#uid").val(), "vid": JSON.stringify(vid), "opera": id, "dest": dest}),
        xhrFields: {withCredentials: true},
...
function download(chooseNum) {
    ...
    form.attr("action", "http://47.110.83.145:8086/file/download");
    var input1 = $("<input>");
                        ...
                        if (g.respCode == 0) {
                            ...
                            $.ajax({
                                ...
                                success: function (h) {
                                    if (h.respCode == 1) {
                                        alert("修改成功");
                                        // location.href = "http://47.110.83.145:8082/"
                                    } else {
                                        alert(h.respMsg);
                                    }
                                },
...
function createShare(chooseNum) {
    ...
    $.ajax({
        url: "http://47.110.83.145:8086/share/share",
        type: "post",
        contentType:'application/json;charset=UTF-8',
        data: JSON.stringify({
            "uid": $("#uid").val(),
        ...
        success: function (data) {
            if (data.respData.indexOf(",") > -1) {
                ...
                $(".share-url").val("http://47.110.83.145:8086/share/s/" + data.respData.substring(0, data.respData.lastIndexOf(",")));
                $(".share-password").val(data.respData.substring(data.respData.lastIndexOf(",") + 1, data.respData.length));
                $(".copyPrivate").val("http://47.110.83.145:8086/share/s/" + (data.respData.substring(0, data.respData.lastIndexOf(","))) + " 提取密码:" + data.respData.substring(data.respData.lastIndexOf(",") + 1, data.respData.length))
            } else {
                $(".share-validity-tip").css("right", "115px");
                $(".create-link").addClass("public-link has-create");
                $("#createShare").css("display", "none");
                $("#cel").children("span").children("span").html("关闭");
                $(".share-url").val("http://47.110.83.145:8086/share/s/" + data.respData);
            }
        },
        error: function () {
            alert("服务器错误")
