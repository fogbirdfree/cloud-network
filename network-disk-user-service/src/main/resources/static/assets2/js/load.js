$(document).ready(function () {
    ...
    $("#up").html5uploader({
        ...
        url: "http://47.110.83.145:8086/file/uploadFile",
        ...
        $("#my-prompt1").modal({
            closeViaDimmer: 0, onConfirm: function (e) {
                ...
                if (inputText.length < 20 && inputText.match(/^[a-zA-Z0-9\u4e00-\u9fa5_]+$/) != null) {
                    $.ajax({
                        url: "http://47.110.83.145:8086/core/createDir",
                        type: "post",
                        data: JSON.stringify({
                            "uid": $('#uid').val(),
                            "dirName": inputText,
            ...
            closeViaDimmer: 0, onConfirm: function (e) {
                ...
                if (inputText == eval($.parseJSON(content[chooseNum])).fileName.substring(0, eval($.parseJSON(content[chooseNum])).fileName.lastIndexOf(".")) && fileType != 0 || inputText == eval($.parseJSON(content[chooseNum])).fileName && fileType == 0) {
                    ...
                    if (inputText.length < 50 && inputText.match(/^[a-zA-Z0-9\u4e00-\u9fa5_()]+$/) != null) {
                        $.ajax({
                            url: "http://47.110.83.145:8086/core/renameFileOrDir",
                            type: "put",
                            data: JSON.stringify({
                                "uid": $("#uid").val(),
                                "newName": inputText,
    ...
    $("#del").click(function () {
        $("#my-confirm2").modal({
            closeViaDimmer: 0, onConfirm: function (options) {
                ...
                $.ajax({
                    url: "http://47.110.83.145:8086/core/deleteFile",
                    type: "delete",
                    data: {"uid": $("#uid").val(), "vid": JSON.stringify(vid)},
                    xhrFields: {withCredentials: true},
                    crossDomain: true,
                    dataType: "json",
                    success: function (data) {
